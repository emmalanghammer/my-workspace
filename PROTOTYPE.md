...
| [`screens/tenants.html`](screens/tenants.html) | **Tenants, a separate screen, linked only.** The Tenants Register + Tenant Details prototype (scoreboard, Leases/Contacts/History/Transactions/Recurring Charges tiles, an Eviction tile, and the Orion AI quick filter), built in another session and brought in here whole. Reached from the Rental Info card on `index.html` and from Rental Info → General → Tenants in the mega menu. It is **not** built on this prototype's `assets/` layer: it is a single self-contained file carrying its own tokens, its own icon sprite and its own CSS, and it shares only `assets/megamenu.css` / `assets/megamenu.js` so all three screens open the identical Full Menu. Nothing inside it was changed on the way in beyond repointing those two paths, so it behaves exactly as it did standalone. See "Worth raising" for the consistency debt this leaves. |
...
| The welcome lines are a literal #575353, not a token | `.ows-welcome` | The frame's two greeting lines carry an unbound #575353: the type styles (`Web/Heading/L/Regular`, `Web/Heading/M/Regular`) are real, the colour is not. This screen originally used `--text-primary` (#666666) on the grounds that tokens are the source of truth; Emma asked on 2026-09-16 for the text to match the frame exactly, so the hex is now literal. If a Foundations variable ever carries #575353, this becomes a token. |
...
## Every name but Tony's is invented

Emma's call, 2026-09-11: *randomize every name except for Tony*, and then,
when the first pass kept three first names on purpose, *use ones that have
never been used before, completely random*.

The prototype's user list had arrived carrying what read as real colleagues'
names. Every one of them is now an invented person, in the data, in the note
authors, in the mention authors and in the text of the system notes. **Tony
...
- **Earlier sections of this file now read strangely in one place.** The story
  ...
  people sharing initials, Charlie Apegian and Carly Anderson, as was. Their
  replacements do not share initials, so that passage describes a real problem
  in names that no longer demonstrate it. The problem was real and the fix
  (keys, not initials) is what matters; the anecdote is just harder to picture
  now.

Tenants, prospects and owners are untouched: Daniel Smith, Marcia Clark,
...
Emma's call, 2026-09-11. There were 633 of them across the prototype and this
file. There are none now, in the copy, in the code comments, in the icon
sheets' headers or here.

The visible copy was rewritten a line at a time rather than swapped, because an
em dash usually wants a different mark depending on what it was doing: a colon
where it introduced ("Verified: the same $45 pet fee"), a full stop where it
joined two sentences ("...until the payables run clears. Two invoices posted
late on the 9th."), a comma where it was parenthetical, and the word "for"
where it was really a label ("Draft renewal for Tomas Webber", "Post 3-day
notice for Flagstone Lot 3"). The page titles use a pipe.

One that was not punctuation at all: the due-date field used an em dash as its
empty value. It reads "mm/dd/yy" now, which is what the quick-add bar already
showed for the same state.
...
## A task mention opens the task on the register

Emma's call, 2026-09-14, arrived at in three steps, which is worth recording
because the middle one is built and then removed.

It started by setting `location.href` to the register **with History / Notes
open on top** of the task. Two changes off that:
...
frame, its markup and the `postMessage` handshake were removed with step 3
